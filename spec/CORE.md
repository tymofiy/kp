...
├── entities.md        # DEPRECATED (since v0.7.7) — use extensions.entities; see EXTENSIONS.md
...
| `spec_version` | string | Version label of the KP:1 spec this pack conforms to (typically a tag, e.g., `v0.8.2-preview`). Pairs with `spec_uri` to pin against a specific revision. |
...
1. Views contain **no knowledge that is not in claims.md**. If a view states a fact, a corresponding claim SHOULD exist. (As of v0.8.2-preview, this is editorial discipline — the conformance runner does not detect "view-laundering" patterns where a view introduces unsupported assertions. Authors are responsible; see [AUTHORING.md §11](AUTHORING.md) anti-pattern #4.)
